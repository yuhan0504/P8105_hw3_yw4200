...
items_number_in_aisles
ggsave("The number of items in different aisles.png",plot = items_number_in_aisles)
```
...
two_panel_plot
ggsave("The Distribution of Data Value in NY State.png", plot = two_panel_plot)
```
## Problem 3
...
age_distribution_plot <- ggplot(demo_clean, aes(x = education, y = age)) +
  geom_boxplot(aes(fill = education),color = "black",alpha = 0.6) +
  facet_wrap(~ sex, ncol = 2) +
  labs(title = "The age distributions",
       x = "Education Level",
       y = "Age")  +
  theme(axis.text.x = element_text(size = 5.3))

age_distribution_plot
ggsave ("The age distributions.png" , plot = age_distribution_plot)
...
# make the three panel plot
 three_panel_plot <- ggplot(merge_data, aes(x = minute, y = mims_value, color = sex)) +
  geom_line(alpha = 0.3) + 
  geom_smooth(alpha = 0.7, size = 0.5) +
